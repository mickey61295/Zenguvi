...
async function getData(){
	...
	"headers": {
		...
		"x-rapidapi-key": "a5f5fe63e8msh7e37b0466d79e9cp1576a3jsn8568c7d5e78d"
	}
    });
    var parsedData = await data.json();
    createBoxes(parsedData);
}
...
function createBoxes(data){
    ...
    for (i of data){
        ...
        box.innerHTML = `
        <img class="thumbnail" src="${i.thumbnail}" alt="${i.title}">
        <div class="text-container">
        <a href="${i.game_url}" target="_blank">${i.title}</a>
        <p>${i.short_description}</p>
        <p>Platforms: ${i.platform}</p>
        </div>`
